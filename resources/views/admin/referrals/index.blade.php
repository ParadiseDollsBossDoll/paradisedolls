                                ...
                                <tr>
                                    <td class="align-middle">
                                        <div class="flex items-center gap-3">
                                            <div class="relative flex h-11 w-11 shrink-0 items-center justify-center overflow-hidden rounded-2xl border border-boss-gold/[0.22] bg-[radial-gradient(circle_at_top,rgba(201,169,110,0.32),rgba(19,15,18,0.94)_70%)] font-display text-sm text-boss-gold-light">
                                ...
                                @elseif ($referral->status !== \App\Models\ModelReferral::STATUS_REJECTED)
                                    @if ($referral->candidate_email)
                                        <form method="POST" action="{{ route('admin.applications.referrals.convert', $referral) }}">
                                            @csrf
                                            <button type="submit" class="rounded-xl bg-boss-gold px-3 py-2 text-[0.68rem] font-semibold text-boss-ink transition hover:opacity-90">{{ __('Convert') }}</button>
                                        </form>
                                    @else
