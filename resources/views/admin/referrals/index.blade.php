    ...
    <div class="mx-auto max-w-[1500px] space-y-7 text-boss-ivory">
        ...
        <section class="grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
            <div class="rounded-2xl border border-white/[0.06] bg-boss-panel-strong p-5 shadow-[0_18px_45px_rgba(0,0,0,0.2)]">
                ...
                <p class="mt-3 font-display text-3xl text-[#F3C3CF]">{{ number_format($summary['total']) }}</p>
            ...
            <div class="rounded-2xl border border-white/[0.06] bg-boss-panel-strong p-5 shadow-[0_18px_45px_rgba(0,0,0,0.2)]">
                ...
                <p class="mt-3 font-display text-3xl text-[#F3C3CF]">{{ number_format($summary['active_referrers']) }}</p>
            ...
            <div class="rounded-2xl border border-white/[0.06] bg-boss-panel-strong p-5 shadow-[0_18px_45px_rgba(0,0,0,0.2)]">
                ...
                <p class="mt-3 font-display text-3xl text-[#F3C3CF]">{{ number_format($summary['leads']) }}</p>
            ...
            <div class="rounded-2xl border border-white/[0.06] bg-boss-panel-strong p-5 shadow-[0_18px_45px_rgba(0,0,0,0.2)]">
                ...
                <p class="mt-3 font-display text-3xl text-[#F3C3CF]">{{ number_format($summary['joined']) }}</p>
                <p class="mt-1 text-xs text-boss-ivory/[0.32]">{{ __('Approved referred models') }}</p>
            </div>

            <div class="rounded-2xl border border-[#EEB4C3]/15 bg-[#EEB4C3]/[0.07] p-5 shadow-[0_18px_45px_rgba(0,0,0,0.2)]">
                <p class="text-[0.65rem] uppercase tracking-[0.18em] text-[#EEB4C3]/70">{{ __('Rewards Due') }}</p>
                <p class="mt-3 font-display text-3xl text-[#F3C3CF]">{{ number_format($summary['eligible_rewards']) }}</p>
            ...
            <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                ...
                <form method="GET" action="{{ route('admin.referrals.index') }}" class="flex w-full flex-col gap-3 lg:max-w-3xl">
                    ...
                    <label for="with-referrals" class="flex w-fit items-center gap-2 text-xs text-boss-ivory/[0.42]">
                        <input id="with-referrals" type="checkbox" name="with_referrals" value="1" class="rounded border-white/15 bg-white/5 text-[#EEB4C3] focus:ring-[#EEB4C3]" @checked($onlyWithReferrals)>
                                ...
                                <tr>
                                    <td class="align-middle">
                                        <div class="flex items-center gap-3">
                                            <div class="relative flex h-11 w-11 shrink-0 items-center justify-center overflow-hidden rounded-2xl border border-[#EEB4C3]/[0.22] bg-[radial-gradient(circle_at_top,rgba(238, 180, 195, 0.32),rgba(19,15,18,0.94)_70%)] font-display text-sm text-[#F3C3CF]">
                                    ...
                                    <td class="align-middle">
                                        @if ($referrer->referral_code)
                                            <div class="space-y-1">
                                                <p class="font-mono text-xs text-[#F3C3CF]">{{ $referrer->referral_code }}</p>
                                                <a href="{{ route('apply', ['ref' => $referrer->referral_code]) }}" target="_blank" rel="noopener" class="text-[0.68rem] text-boss-ivory/[0.34] transition hover:text-[#EEB4C3]">
                                                    {{ __('Open link') }}
                                                </a>
                                            </div>
                                        @else
                                    ...
                                    <td class="align-middle text-center font-display text-xl text-[#F3C3CF]">{{ number_format($referrer->referrals_count) }}</td>
                                    ...
                                    <td class="align-middle text-center">
                                        <span class="rounded-full border border-[#EEB4C3]/15 bg-[#EEB4C3]/[0.07] px-2.5 py-1 text-[0.65rem] text-[#F3C3CF]">
                ...
                @forelse ($recentReferrals as $referral)
                    @php
                        $statusClasses = match ($referral->status) {
                            ...
                            \App\Models\ModelReferral::STATUS_PENDING => 'border-[#EEB4C3]/20 bg-[#EEB4C3]/10 text-[#F3C3CF]',
                        ...
                        <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                            <div class="min-w-0">
                                <div class="flex flex-wrap items-center gap-2">
                                    ...
                                    <span class="rounded-full border border-[#EEB4C3]/15 bg-[#EEB4C3]/[0.07] px-2.5 py-1 text-[0.65rem] text-[#F3C3CF]">{{ $referral->rewardStatusLabel() }}</span>
                                </div>
                                ...
                                <div class="mt-3 flex flex-wrap gap-2 text-[0.68rem] text-boss-ivory/[0.34]">
                                    <span>{{ __('Referred by') }} <span class="text-[#F3C3CF]">{{ $referral->referrer?->name }}</span></span>
                            ...
                            <div class="flex flex-wrap gap-2 lg:justify-end">
                                ...
                                @if ($referral->reward_status === \App\Models\ModelReferral::REWARD_ELIGIBLE)
                                    <form method="POST" action="{{ route('admin.applications.referrals.reward-paid', $referral) }}">
                                        @csrf
                                        <button type="submit" class="rounded-xl border border-[#EEB4C3]/25 bg-[#EEB4C3]/10 px-3 py-2 text-[0.68rem] font-semibold text-[#EEB4C3] transition hover:bg-[#EEB4C3] hover:text-boss-ink">{{ __('Mark Paid') }}</button>
                                    </form>
                                @endif
                            </div>
                        </div>
