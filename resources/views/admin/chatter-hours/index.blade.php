                                ...
                                <div>
                                    ...
                                    <div class="mt-3 grid gap-3 lg:grid-cols-2">
                                        @foreach($assignments as $assignment)
                                            <form method="POST" action="{{ route('admin.chatter-hours.chatters.roles', $chatter) }}" class="grid gap-3 rounded-lg border border-white/[0.06] bg-white/[0.02] p-3 sm:grid-cols-[minmax(0,1fr)_8rem_auto] sm:items-start">@csrf
                                                ...
                                                <button class="pd-btn-secondary h-[43px] w-full rounded-lg px-3 text-xs sm:mt-5 sm:w-auto">{{ __('Save') }}</button>
                                            </form>
                                        @endforeach
                                    </div>
                                    <form method="POST" action="{{ route('admin.chatter-hours.chatters.roles', $chatter) }}" class="mt-3 grid gap-3 rounded-lg border border-dashed border-boss-gold/20 p-3 sm:grid-cols-[minmax(0,1fr)_8rem_auto] sm:items-start">@csrf<input type="hidden" name="is_active" value="1"><label class="block min-w-0"><span class="pd-label">{{ __('Add another role') }}</span><input class="pd-input mt-1 w-full min-w-0" name="work_role_name" list="chatter-work-role-options" placeholder="{{ __('Role name') }}" required></label><label class="block min-w-0"><span class="pd-label">{{ __('USD/hr') }}</span><input class="pd-input mt-1 w-full" type="number" name="hourly_rate" step="0.01" min="0" value="{{ number_format(($rate?->base_rate_pence ?? 0) / 100, 2, '.', '') }}" required></label><button class="pd-btn-primary h-[43px] w-full rounded-lg px-3 text-xs sm:mt-5 sm:w-auto">{{ __('Assign role') }}</button></form>
                                </div>
