        ...
        <div x-show="createOpen" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4" @keydown.escape.window="createOpen = false">
            ...
            <div class="relative max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-lg border border-white/[0.08] bg-boss-panel p-6 shadow-2xl">
                ...
                <form method="POST" action="{{ route('admin.chatter-hours.chatters.store') }}" class="mt-6 grid gap-4 sm:grid-cols-2">@csrf
                    <label><span class="pd-label">{{ __('Name') }}</span><input class="pd-input mt-2" name="name" required></label><label><span class="pd-label">{{ __('Email') }}</span><input class="pd-input mt-2" type="email" name="email" required></label><label><span class="pd-label">{{ __('Timezone') }}</span><input class="pd-input mt-2" name="timezone" value="Europe/London" required></label><label><span class="pd-label">{{ __('Chatter USD/hr') }}</span><input class="pd-input mt-2" type="number" step="0.01" name="base_hourly_rate" value="3.00" required></label><label><span class="pd-label">{{ __('Overtime after hours') }}</span><input class="pd-input mt-2" type="number" step="0.25" name="overtime_threshold_hours" value="40" required></label><label><span class="pd-label">{{ __('Overtime multiplier') }}</span><input class="pd-input mt-2" type="number" step="0.01" name="overtime_multiplier" value="1.5" required></label><label><span class="pd-label">{{ __('Night premium') }}</span><input class="pd-input mt-2" type="number" step="0.01" name="night_premium_multiplier" value="1.2" required></label><label><span class="pd-label">{{ __('Weekend premium') }}</span><input class="pd-input mt-2" type="number" step="0.01" name="weekend_premium_multiplier" value="1.25" required></label><label><span class="pd-label">{{ __('Night starts') }}</span><input class="pd-input mt-2" type="time" name="night_starts_at" value="22:00" required></label><label><span class="pd-label">{{ __('Night ends') }}</span><input class="pd-input mt-2" type="time" name="night_ends_at" value="06:00" required></label><label><span class="pd-label">{{ __('Effective from') }}</span><input class="pd-input mt-2" type="date" name="effective_from" value="{{ now('Europe/London')->toDateString() }}" required></label><div class="flex items-end justify-end gap-2"><button type="button" class="pd-btn-secondary rounded-lg px-4 py-2.5 text-xs" @click="createOpen = false">{{ __('Cancel') }}</button><button class="pd-btn-primary rounded-lg px-4 py-2.5 text-xs">{{ __('Create and invite') }}</button></div>
                </form>
            </div>
        </div>
        @endif
