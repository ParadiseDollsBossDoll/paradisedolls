    ...
    <div class="mx-auto max-w-6xl space-y-8 text-boss-ivory">
        ...
        <section class="grid grid-cols-2 gap-4 lg:grid-cols-4">
            @foreach ([
                ...
                <a href="{{ $stat[3] }}" class="pd-stat block transition-all duration-200 hover:border-[#EEB4C3]/25 hover:shadow-glow">
                    <p class="font-display text-[2.25rem] leading-none text-[#EEB4C3]">{{ $stat[1] }}</p>
        ...
        <section class="grid gap-6 lg:grid-cols-2">
            <div class="pd-panel-strong p-6">
                <div class="mb-5 flex items-center justify-between">
                    ...
                    <a href="{{ route('admin.applications.index') }}" class="text-[0.72rem] text-[#EEB4C3] hover:text-[#F3C3CF]">{{ __('Review') }} -></a>
                </div>
                ...
                <div class="space-y-3">
                    @forelse ($recentApplications as $application)
                        <div class="flex items-center gap-3 rounded-sm border border-white/[0.05] bg-white/[0.025] p-3">
                            ...
                            <span class="rounded-full px-2 py-0.5 text-[0.62rem] capitalize {{ $application->status === \App\Models\ModelApplication::STATUS_PENDING ? 'bg-[#EEB4C3]/10 text-[#EEB4C3]' : ($application->status === \App\Models\ModelApplication::STATUS_APPROVED ? 'bg-green-400/10 text-green-300' : 'bg-red-400/10 text-red-300') }}">
            ...
            <div class="pd-panel-strong p-6">
                <div class="mb-5 flex items-center justify-between">
                    ...
                    <a href="{{ route('admin.courses.create') }}" class="text-[0.72rem] text-[#EEB4C3] hover:text-[#F3C3CF]">{{ __('New Course') }} -></a>
                </div>

                <div class="space-y-4">
                    @forelse ($recentCourses as $course)
                        <a href="{{ route('admin.courses.edit', $course) }}" class="group block rounded-sm border border-white/[0.05] bg-white/[0.025] p-4 transition-colors hover:border-[#EEB4C3]/20">
                            ...
                            <div class="flex items-center justify-between gap-4">
                                <p class="truncate text-[0.88rem] text-boss-ivory group-hover:text-[#F3C3CF]">{{ $course->title }}</p>
