...
<div class="mx-auto max-w-7xl space-y-5 text-boss-ivory">
    ...
    <section class="rounded-md border border-white/[0.08] bg-white/[0.025] p-5"><div class="flex flex-wrap items-end justify-between gap-3"><div><p class="pd-kicker">{{ __('Current Week') }}</p><h2 class="mt-1 font-display text-2xl">{{ $currentTimesheet->period_start->format('j M') }} - {{ $currentTimesheet->period_end->format('j M Y') }}</h2></div><span class="rounded-full border border-white/10 px-3 py-1 text-xs text-boss-ivory/50">{{ $currentTimesheet->statusLabel() }}</span></div>
        <div class="mt-5 overflow-x-auto"><table class="pd-table min-w-[820px]"><thead><tr><th>{{ __('Date') }}</th><th>{{ __('Work role') }}</th><th>{{ __('Clock in') }}</th><th>{{ __('Clock out') }}</th><th>{{ __('Hours Worked') }}</th><th>{{ __('Status') }}</th></tr></thead><tbody>@forelse($currentShifts as $shift)<tr><td>{{ $shift->clocked_in_at->timezone($tz)->format('D, j M') }}</td><td><span class="font-medium">{{ $shift->workRole?->name ?? __('Chatter') }}</span><span class="mt-0.5 block text-xs text-boss-ivory/35">${{ number_format(($shift->hourly_rate_pence ?? 0) / 100, 2) }} USD/hr</span></td><td>{{ $shift->clocked_in_at->timezone($tz)->format('g:i A') }}</td><td>{{ $shift->clocked_out_at?->timezone($tz)->format('g:i A') ?? __('Active') }}</td><td>{{ $formatMinutes((int) $shift->getAttribute('worked_minutes')) }}</td><td>{{ $shift->isOpen() ? ($activeBreak && $openShift?->is($shift) ? __('On break') : __('In progress')) : __('Recorded') }}</td></tr>@empty<tr><td colspan="6" class="text-center text-boss-ivory/40">{{ __('No shifts recorded this week.') }}</td></tr>@endforelse</tbody></table></div>
    </section>
