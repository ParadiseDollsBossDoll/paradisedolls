...
return new class extends Migration
{
    ...
    {
        if (! $this->usesMysql() || ! Schema::hasTable('chatter_shifts') || ! Schema::hasTable('chatter_breaks')) {
            return;
        }

        // MariaDB can implicitly add ON UPDATE CURRENT_TIMESTAMP to the first
        // non-null TIMESTAMP column. Event times must never change on updates.
        DB::statement('ALTER TABLE chatter_shifts MODIFY clocked_in_at DATETIME NOT NULL, MODIFY clocked_out_at DATETIME NULL');
        DB::statement('ALTER TABLE chatter_breaks MODIFY started_at DATETIME NOT NULL, MODIFY ended_at DATETIME NULL');

        if (! Schema::hasTable('chatter_time_audits')) {
            return;
        }
        ...
        DB::table('chatter_shifts')->orderBy('id')->eachById(function (object $shift): void {
            ...
            foreach ($breakRows as $index => $breakRow) {
                ...
                $breakUpdate = ['started_at' => $period['started_at']];
                if ($period['ended_at'] !== null) {
                    $breakUpdate['ended_at'] = $period['ended_at'];
                }

                DB::table('chatter_breaks')->where('id', $breakRow->id)->update($breakUpdate);
            }
        });
    }
    ...
    {
        // Intentionally irreversible. Restoring TIMESTAMP would restore the
        // MariaDB behavior that corrupts immutable clock and break events.
    }
