...
class GbpUsdExchangeRateService
{
    public const SETTINGS_KEY = 'chatter_payroll_gbp_usd_currency';

    private const FAILURE_CACHE_KEY = 'chatter_payroll_gbp_usd_currency.refresh_failed';
    ...
    public function refresh(): array
    {
        ...
        try {
            ...
            if ($rate <= 0 || $rate > 100) {
                throw new RuntimeException('The exchange-rate provider returned an invalid GBP/USD rate.');
            }

            $details = [
                'gbp_to_usd_rate' => number_format($rate, 4, '.', ''),
            ...
            Log::warning('Automatic GBP/USD exchange-rate refresh failed; using the last known rate.', [
    ...
    {
        $rate = $details['gbp_to_usd_rate']
            ?? config('services.chatter_payroll.gbp_to_usd_rate_fallback', '1.2700');
